...
## Drinking Age
Based on reports from previous literature, drinking age seems to be a state's first-line of defense against alcohol-related traffic fatalities (Lovenheim & Slemrod, 2010). Preliminary data exploration shows that by far, the most common drinking age is 21, which is significantly related to the rate of alcohol-related traffic fatalities (see table below). State, however, also appears to be significant. This suggests that other state laws are significantly affecting drunk driving. Since the drinking age is now 21 across the nation, this variable does not contribute actionable information to this study. The other state laws may still be actionable and warrant further study. Diagnostics confirm that distribution of drinking age is not normal. Additionally, there is an outlier with undue leverage (Louisiana in 1988).
...
Because our goal is to have a correct model, we are choosing only explanatory variables which have a significant relationship with the rate of alcohol-related traffic fatalities. This model may be smaller than ones which have the goal of prediction. AIC may overfit correct models and BIC penalizes large models even more, so we choose the model that lowers BIC the most.

## Subset Selection\
Subset selection does not evaluate all possible models; however, it is faster and does not cost as much because not all subsets are calculated. We chose to use Forwards Backwards Selection because we want a correct model which means having a smaller one. Of course, underfitting a model is not ideal however between underfitting and overfitting, underfitting the model is more likely to achieve the smaller and often more correct model which Forwards Selection or Forwards Backwards Selection is more likely to do. Forwards Backwards Selection does not underfit as much as Forwards Selection, so we used Forwards Backwards as our subset selection. These were the models with the lowest BIC when looking at all possible models.
...
This data does not seem to be normally distributed as there are heavy tails on all three models. There are outliers and the points towards the end of the line are not where the points would sit if the data was totally normally distributed.
...
$H_{o}:$ errors are normal   $H_{A}:$ errors are non-normal

   $P-Value_{model\ 1}:  0.2426$     $P-Value_{model\ 2}:  0.02156$     $P-Value_{model\ 3}:  0.0024$

The p-values for model 1 and 2 are greater than an alpha of 0.01. Thus we fail to reject the null and conclude that the errors are normally distributed. However, for model 3 the p-value is less than an alpha of 0.01, 0.1 and 0.05 thus we reject the null and conclude that the errors are not normally distributed.
...
**Model 3:** $$ ln(Y_{1988}) = -0.3367+ 0.0190X_{region, other} + 0.1032X_{region, south} - 0.0001X_{income} + 0.0001 X_{miles}$$
...
(ii) **Model 3:** $\hat{\beta_{3}}$ Souhtern states had  11087 more alcohol fatalities on average compared to  northeastern states, holding all other variables constant.


## Fixed Regression Models for State Laws

To assess whether state implemented laws had a significant effect on alcohol related vehicle fatalities, we first subset the data. It is unrealistic to expect the demographic and political priorities of each state is equivalent. In a attempt to address this discrepancy, we have decided to investigate the effectiveness of jail, service, and breath laws in the Southern and Northeastern regions of the United States.

In the case of this model, jail and service indicate whether a first-time drunk driving offense leads to subsequent jail time or mandatory community service. Since both service and jail time are regarded as punishments, they are combined into a single variable called penalty. The breath factor indicates which specific state currently has a law in place to allow officers to perform breath tests on drivers they pulled over. Intuitively, each of these factors appear to deter drunk driving under the assumption that the information from each state is homogeneous and fully reported. In many ways, this is wishful thinking, as there are factors not included in this data set such as vehicle sales, road quality, and number of bars per area, etc,.

The following graphs illustrate this point. In most all cases, once a state implemented a breath test or penalty law, it remained in effect the following years. The state fatality rates were graphed for the starting year 1982 and ending year 1988. The points are color coded to indicate whether a breath test or penalty law is in place and shape coded to represent the year. There is no clear pattern on whether either of these laws had any effect when compared to other states. In other words, we cannot determine the net effect of either law on fatality rates between even neighboring states.
...
Fortunately, the fixed effects regression model can be implemented to determine the net effect of breath tests and penalty laws by holding state and year constant. The fixed effect model seeks to account for variability within year and state, and allows us to work with data which changes overtime while reducing the bias which results from data without said change. As a result, we assume there are influential factors within each state and year are not accounted for in the observed data. For this model, we will be predicting on the log transformation of the fatality rate.

In the fixed model:

$$ln(Y_{it}) = \beta_0 + \beta_1 X_{1,it} +\cdots+\beta_1 X_{k,it} + \gamma_2 E2 +\cdots + \gamma_n En + \delta_2 D2 + \cdots +  \delta_T DT + u_{it} .$$

where $t=1,...,T$ represents the year (time) and $i=1,...,n$ represents the collection of instances for each state $i$ (entity).

There are n-1 dummy variables for states, and T-1 dummy variables for years.
...
$u_{it}$ represents the error terms and $\beta_1 X_{k,it}$ represents the $k^th$ predictor.

For the fixed effects model, the following assumptions are made:

- The error terms when conditioned on the predictors, have a mean equal to zero, that is, $E(u_{it}|X_{1,it},\cdots,X_{k,it})=0$
- The predictor terms and errors for all $i=1,\cdots,n$ are $i.i.d.$ from their joint distribution.
...
- There are not significant outliers
 ...
 A simpler, project specific version of the model using only BreathTest and Penalty for regressiors is:
  ...
  $$\hat{FatalityRate}_{State, Year} = \hat{\beta_{1}}  X_{Penalty :\ Yes} + \hat{\beta_{2}}  X_{Breath \ Test: \ Yes} + StateEffects_{State} + TimeEffects_{Year}+ errors_{State, Year}$$
 ...
 The results of a fixed linear regression model on all states shows no significant effect of breath tests or penalty laws on drunk driving. However, as noted prior, it is unlikely that even a panel model on all fifty-states can correctly adjust for the unobsrved differences between each state and year.
...
The fixed regression model using southern states indicates that a Penality does effect fatality rates to a significance level of $\alpha=0.05$. Unfortunately, the existence of a Penality appears to increase fatality rates while BreathTests are insignificant.
...
mtext("Diagnostic Plots Northeastern States", outer = TRUE, cex = 1.5)

```

## Beer Taxes and Alcohol Consumption
...
For these variables, we have two hypotheses about their affects on one another:

  1. Beer taxes will have a *positive relationship* with spirit consumption; As taxes on beer increase, one becomes deterred from purchasing beer, and their likelihood of alternatively choosing spirits increases.
...
The p-value for the interaction term in our reduced model is `r round(full_coef[4,4], digits=2)`, so there is enough evidence to support that the interaction term has no effect and we should use the reduced model.
...
Plotting the diagnostic measures to see if there is normality in our residuals and the variance in our error terms is constant:
...
Notably our QQ plot has a heavy tail, and we will validate that in the next section.

#### Test for Normality (Shapiro Wilks Test)

$H_0$: Error terms are normal, $H_A$: Error terms are not normal

```{r, echo = FALSE}
ppp <- check.normality.assumptions(reduced_model)$`p-value`
```

Our p-value for the Shapiro Wilks test is `r round(ppp, digits=3)` so fail to reject the null hypthesis at an $\alpha$ of 0.01 and conclude that the errors are normally distributed.

#### Test for Constant Variance (Breusch-Pagan Test)

$H_0$: Residuals have equal variance, $H_A$: Residuals do not have equal variance

Our p-value for the Breusch-Pagan test is `r round(bptest(reduced_model)$p.value, digits=2)` so we fail to reject the null hypothesis at an $\alpha$ of 0.01 and conclude that our model residuals have equal variance.

### Addressing Concerns and Causality

Although our intitial model identified a relationship between beer taxes and alcohol-related fatality numbers, it is not what we expected.
...
Looking at these pair plots, it appears that there isn't a clear relationship between any of our variables.
To quantify this, we will use a fixed effects linear model, the same as in the "Fixed Regression Models for State Laws" section.
We will be keeping the year and state effects fixed to dismiss any differences unrelated to our parameters:

$$Y_{ij} = \beta_0 + \beta_1 X_{beertax} + \epsilon_{ij}$$

Where $Y$ is the YoY difference in number of alcohol-related fatalities per 10k citizens, $\beta_0$ is the fixed effects for year and state, $\beta_1$ is the effect of YoY difference in beer taxes, $X_1$ is the YoY difference in beer taxes and $\epsilon$ is our error term, i.i.d. $N(0, \sigma_2)$.
$i = 1,...,n$ where $n$ is the number of states and $j = 1,...,m$ where $m$ is the number of YoY changes (one less than the number of years).
...
# summary(beertax_plm)$coefficients
```

Testing our beertax coefficient $\hat{\beta_1}$ we can see a p-value of `r round(coeftest(beertax_plm, vcov=vcovHC, type="HC1")[1,4], digits=2)` so we can say that **there is a non-causal relationship between beer taxes and alcohol related traffic fatalities**.
This lines up with other findings (Mast et al., 1999) based on this dataset, keeping our conclusion consistent.
...
Since the nation-wide drinking age in the United States is now 21, drinking age is no longer a relevant strategy for drunk driving prevention. Beer tax does not seem to impact alcohol fatalities, which reflects the findings in previous literature (Mast et al. 1999). They found that beer tax does not impact the heaviest drinkers, which is why it may not translate well as a prevention. Spirit consumption had no relation to the alcohol fatality rate in this study. This could discourage policy-makers from focusing on spirit-related laws to decrease drunk driving fatalities. 
Model-finding analysis show that spirits were more important at the beginning of the decade, but waned towards the end of the decade. 
Overall, both the breath test and penalty laws are insignificant when looking at the United States as a whole. Since each state is so different, further analysis focused on the regions. The south had significantly increased fatality rates with increased penalties. There is no found causitive effect between the two, and the reason behind this finding requires further investigation. In the northeastern region, both breath test and penalty laws were significantly negatively correlated with fatalities. This might be because the states are more densly populated in the northeast, so police officers could have a higher chance of encountering a drunk driver, whereas in the south drunk drivers may not encounter a police officer in a sparsly-populated area.

Alcohol-related traffic fatalities are a public health/epidemiology issue not just a public policy issue. Preventing them clearly requires more than changing a drinking age, increasing taxes, or administering penalties. There are likely other factors at play, dealing with addiction and cultural values, that cannot be explored in this study. Psychologists, medicine, addiction and trauma counselors may also have role in this above and beyond policy makers (Perrine et al., 1989).

#### Caveats

This dataset was set up for data exploration in the context of a textbook, not in the context of a specific study or analysis. For example, beer tax and spirit consumption were included, but not beer consumption or spirit tax. Future studies looking at policy changes and drunk driving should consider this in their experimental design, and solicit the needed information. This could mean decreasing the scope and focusing on one state, or a series of counties. By including more specific measurements and incorporating the necessary considerations into the experimental design could lead to stronger conclusions.


# Acknowledgement {-}

This project has only been discussed within the group, and with the professor. Inspiration for the map figure comes from the @littlemissdata blog. Dr. Erin Melcon’s STA 108/106 R handouts and lecture notes were referenced in this project.
