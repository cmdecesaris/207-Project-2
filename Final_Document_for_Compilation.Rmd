...
- There are not significant outliers
 ...
 A simpler, project specific version of the model using only BreathTest and Penalty for regressiors is:
  ...
  $$\hat{FatalityRate}_{State, Year} = \beta_{1}^{\hat}  X_{Penalty :\ Yes} + \beta_{2}^{\hat}  X_{Breath \ Test: \ Yes} + StateEffects_{State} + TimeEffects_{Year}+ errors_{State, Year}$$
...
Looking at these pair plots, it appears that there isn't much of a relationship between any of our variables.
...
This lines up with other findings (Mast, B., Benson, B., & Rasmussen, D.) based on this dataset, keeping our conclusion consistent.
