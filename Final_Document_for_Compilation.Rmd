...
Where $Y$ is the YoY difference in number of alcohol-related fatalities per 10k citizens, $\beta_0$ is the fixed effects for year and state, $\beta_1$ is the YoY difference in beer taxes and $\epsilon$ is our error term.
...
Testing our beertax coefficient we can see a p-value of `r round(coeftest(beertax_plm, vcov=vcovHC, type="HC1")[1,4], digits=2)` so we can say that *there is a non-causal relationship between beer taxes and alcohol related traffic fatalities*.
